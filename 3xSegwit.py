...
def seek(r, df_handler):
	...
	while True:
		...
		publ_addr_b = bytes.decode(base58.b58encode_check(b'\x05' + hash160b))
		priv = WIF.decode()
		pub = publ_addr_b
		...
		pub = pub + '\n'
		filename = '/data/BitBruteForce-Wallet-Compressed-Address/top3.34M_add3x_bal_over1.txt' #<----------------------------------------------------------change here Dir
...
if __name__ == '__main__':
	jobs = []
	df_handler = pd.read_csv(open('/data/BitBruteForce-Wallet-Compressed-Address/top3.34M_add3x_bal_over1.txt', 'r'))  #<--------------------------------------------------------change here Dir
